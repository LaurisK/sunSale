...
def test_tomorrows_error_shows_as_soon_as_its_auction_publishes():
    """Tomorrow has no settled record until midnight, but once its prices are in
    the error is paired from the frozen prediction and the live slots."""
    from custom_components.sun_sale.contract.models import PredictedDay, PriceCurveHistory
    from custom_components.sun_sale.pipeline import price_forecast as pf
    from tests.test_price_forecast import _TODAY, _generation, _price_series

    tomorrow = _TODAY + timedelta(days=1)
    history = _trained_history(days=50)  # ends before _TODAY
    history = PriceCurveHistory(
        records=history.records,
        predictions=(PredictedDay(day=tomorrow, lead_days=1, curve=tuple([0.04] * 24)),),
        shape_state=history.shape_state,
    )
    forecast = pf.compute_price_forecast(
        price_series=_price_series(days=2),  # today + tomorrow, both priced
        history=history,
        weather=None,
        generation=_generation(),
        negative_threshold_eur_kwh=0.02,
        now=datetime.combine(_TODAY, datetime.min.time(), UTC) + timedelta(hours=15),
        local_tz=UTC,
        latitude=54.9, longitude=23.9,
    )
    errors = [e for e in forecast.error_slots if e.start.date() == tomorrow]
    assert len(errors) == 24
    # _price_series: 0.05 base, 0.25 over 17–21 h.
    assert errors[0].error_eur_kwh == pytest.approx(0.01)
    assert errors[18].error_eur_kwh == pytest.approx(0.21)


def test_an_unpriced_tomorrow_publishes_no_error():
    """Before the auction tomorrow is zero-spot filler, never an 'actual'."""
    ...
    tomorrow = _TODAY + timedelta(days=1)
    series = _price_series(days=2)
    series = replace(series, slots=tuple(
        replace(s, spot_eur_kwh=0.0, priced=False) if s.start.date() == tomorrow else s
    ...
    forecast = pf.compute_price_forecast(
        ...
        generation=_generation(),
        negative_threshold_eur_kwh=0.02,
        now=datetime.combine(_TODAY, datetime.min.time(), UTC) + timedelta(hours=8),
        local_tz=UTC,
        latitude=54.9, longitude=23.9,
    )
    assert not [e for e in forecast.error_slots if e.start.date() == tomorrow]
    assert any(p.start.date() == tomorrow for p in forecast.predicted_slots)
