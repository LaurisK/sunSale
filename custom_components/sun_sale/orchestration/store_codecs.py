        ...
        "records": [
            {
                ...
                "p4": r.peak_4h_eur_kwh,
                "t1": r.trough_1h_eur_kwh,
                "t4": r.trough_4h_eur_kwh,
                "nh": r.negative_hours,
                "mean": r.mean_eur_kwh,
    ...
    for r in d.get("records", []):
        try:
            records.append(PriceDayRecord(
                ...
                # Records saved before the 3 h → 4 h widening carry "p3"/"t3",
                # deliberately not read: a 3 h value is not a 4 h one. They keep
                # their other statistics and simply lack the 4 h band.
                peak_4h_eur_kwh=_opt_float(r.get("p4")),
                trough_1h_eur_kwh=float(r["t1"]),
                trough_4h_eur_kwh=_opt_float(r.get("t4")),
